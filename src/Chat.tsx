...
const ChatInner = () => {
  ...
  const [retryTimeout, setRetryTimeout] = useState<number>(() => {
    const stored = Number(localStorage.getItem('retryTimeout'))
    return Number.isFinite(stored) && stored > 0 ? stored : DEFAULT_RETRY_TIMEOUT
  })
  ...
  const retryTimeoutRef = useRef(retryTimeout)
